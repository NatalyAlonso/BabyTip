...
public class NewBabyActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        img_perfil = (ImageView)findViewById(R.id.image_toolbar);
    ...
    private void guardarBebe(){
        ...
        if (isValidBaby) {
            int id = data.addBaby(bebe);

            if(img == null)
                img = BitmapFactory.decodeResource(getResources(), R.drawable.bebe_peque);

            FileOutputStream out = null;
            String filename = ""+id;
            (new File(getFilesDir().getPath()+"/babytip")).mkdirs();
            File file = new File(getFilesDir().getPath()+"/babytip",""+id);
            try {
            ...
            intent.putExtra(BabyTipContract.BabysEntry._ID, id);
            intent.putExtra("selected_index",data.getAllBabys().size()-1);
    ...
    public void showMessage(String message){
        ...
        alertDialogBuilder = new AlertDialog.Builder(NewBabyActivity.this);
